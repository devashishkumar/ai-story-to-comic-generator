...
interface ComicPanelInput {
  panel: number;
  scene: string;
  dialogue: string;
}

interface ComicPanelOutput extends ComicPanelInput {
  imagePrompt: string;
}
...
if (!apiKey) {
  console.error(
    "ERROR: GEMINI_API_KEY is missing. Add it to .env like GEMINI_API_KEY=your_key",
  );
  process.exit(1);
}
...
async function storyToPanels(story: string): Promise<ComicPanelInput[]> {
  ...
  const parsed = JSON.parse(text) as ComicPanelInput[];
  return parsed;
}

async function generateImage(scene: string): Promise<string> {
